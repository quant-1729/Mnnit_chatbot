...
class ActionFallback(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: dict) -> list:
            try:
                ...
                # Performing a Google search and getting the first result
                first_result = next(search(user_query, num_results=1, advanced=True), None)

                # Check if there is a result and send relevant information
                if first_result:
                    response = (
                        f"Here is the information for your search query:\n"
                        f"Title: {first_result.title}\n"
                        f"Description: {first_result.description}\n"
                        f"For more you can see here: {first_result.url}"
                    )
                else:
